...
namespace DUR.OPCUA
{
    public class UaUi
    {
        TreeView _BrowseNodesTV = null;
        ListView _AttributesLV = null;
        UaClient _UaClient = null;
        public void connect(UaClient uaClient, TreeView BrowseNodesTV, ListView AttributesLV)
        {
            _BrowseNodesTV = BrowseNodesTV;
            _AttributesLV = AttributesLV;
            _UaClient = uaClient;

            //add events
            _BrowseNodesTV.MouseDown += BrowseNodesTV_MouseDown;
            _BrowseNodesTV.AfterSelect += BrowseNodesTV_AfterSelect;
            _BrowseNodesTV.BeforeExpand += BrowseNodesTV_BeforeExpand;
        ...
        private void BrowseNodesTV_MouseDown(object sender, MouseEventArgs e)
        {
            try
            {
                _BrowseNodesTV.SelectedNode = _BrowseNodesTV.GetNodeAt(e.X, e.Y);
            }
        ...
        private void PopulateBranch(NodeId sourceId, TreeNodeCollection nodes)
        {
            try
            {
                ...
                ReferenceDescriptionCollection references = UaClientUI_utils.Browse(_UaClient.Session, nodesToBrowse, false);
        ...
        private void DisplayAttributes(NodeId sourceId)
        {
            try
            {
                ...
                for (int ii = 0; ii < results.Count; ii++)
                {
                    ...
                    _AttributesLV.Items.Add(item);
                }

                // adjust width of all columns.
                for (int ii = 0; ii < _AttributesLV.Columns.Count; ii++)
                {
                    _AttributesLV.Columns[ii].Width = -2;
                }
            }
            catch (Exception exception)
            {
